...
function initializeDatabase() {
  db.serialize(() => {
    ...
    ensureColumn('promocion', 'eliminada', 'INTEGER DEFAULT 0');
    ...
    db.run(`CREATE TABLE IF NOT EXISTS admin_rostro (
      id_rostro INTEGER PRIMARY KEY AUTOINCREMENT,
      id_admin INTEGER NOT NULL,
      nombre VARCHAR(100) NOT NULL,
      descriptor TEXT NOT NULL,
      foto_referencia TEXT,
      fecha_registro DATETIME DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY(id_admin) REFERENCES administrador_evento(id_admin) ON DELETE CASCADE
    )`);
    ...
    ensureColumn('producto', 'id_botella_vinculada', 'INTEGER');
    ensureColumn('producto', 'vasos_por_botella', 'INTEGER DEFAULT 10');
    ...
    ensureColumn('comanda', 'es_cortesia', 'INTEGER DEFAULT 0');
    ...
    ensureColumn('configuracion', 'logo_ticket', 'TEXT');
    ensureColumn('mesero', 'es_cortesia', 'INTEGER DEFAULT 0');
    ensureColumn('comanda', 'es_cortesia', 'INTEGER DEFAULT 0');
...
app.post('/api/login/admin-pin', (req, res) => {
  const { password, pin } = req.body;
  const clave = String(password || pin || '').trim();
  if (!clave) return res.status(400).json({ success: false, message: 'Ingresa la clave de administrador.' });

  const queryAdmin = `
    SELECT a.id_admin, a.nombre, a.id_evento, a.rol, e.nombre_evento
    FROM administrador_evento a
    JOIN evento e ON a.id_evento = e.id_evento
    WHERE a.password = ? AND a.activo = 1
    LIMIT 1
  `;
  dbQuery(queryAdmin, [clave], (err, results) => {
    if (err) return res.status(500).json({ error: err.message });
    if (results && results.length > 0) {
      return res.json({
        success: true,
...
app.post('/api/login/mesero', (req, res) => {
  ...
  let query = `
    SELECT m.id_mesero, m.nombre, m.id_cajero, c.id_barra, COALESCE(m.es_cortesia, 0) AS es_cortesia,
           m.password AS pin, m.password AS password, m.usuario
    FROM mesero m
...
function calcularDistanciaEuclidiana(v1, v2) {
  if (!v1 || !v2 || !Array.isArray(v1) || !Array.isArray(v2)) return 999.0;
...
function obtenerDistanciaMinima(liveDesc, rawDescriptorJSON) {
  try {
    const descData = typeof rawDescriptorJSON === 'string' ? JSON.parse(rawDescriptorJSON) : rawDescriptorJSON;
    if (!descData) return 999.0;

    // Support both multi-pose descriptor arrays [[128...], [128...]] and single [128...]
    if (Array.isArray(descData) && descData.length > 0 && Array.isArray(descData[0])) {
      let minDist = 999.0;
      for (const singleDesc of descData) {
        const dist = calcularDistanciaEuclidiana(liveDesc, singleDesc);
    ...
    if (Array.isArray(descData)) {
      return calcularDistanciaEuclidiana(liveDesc, descData);
    }
  } catch (e) {
    console.error('Error al evaluar descriptor facial:', e);
  }
  return 999.0;
}

// Reconocimiento facial de administrador Apple Face ID (activado con PIN 0000 o botón)
app.post('/api/login/admin-facial', async (req, res) => {
  const { liveDescriptor } = req.body;
  if (!liveDescriptor || !Array.isArray(liveDescriptor) || liveDescriptor.length < 64) {
    return res.status(400).json({ success: false, message: 'Captura facial biométrica no válida.' });
  }

  try {
    const rostros = await dbAll(
      `SELECT r.id_rostro, r.id_admin, r.nombre AS rostro_nombre, r.descriptor, a.nombre AS admin_nombre, a.usuario, a.id_evento, e.nombre_evento
       FROM admin_rostro r
       JOIN administrador_evento a ON a.id_admin = r.id_admin
       JOIN evento e ON a.id_evento = e.id_evento
       WHERE a.activo = 1`
    );
    ...
    if (!rostros || rostros.length === 0) {
      return res.status(200).json({
        success: false,
        no_registered_faces: true,
        message: 'No hay rostros de administrador registrados. Entra como Admin con usuario/contraseña y registra tu rostro en la pestaña Configuración.'
      });
    }

    let mejorCoincidencia = null;
    let menorDistancia = 999.0;

    for (const r of rostros) {
      const dist = obtenerDistanciaMinima(liveDescriptor, r.descriptor);
    ...
    // Umbral de distancia euclidiana estricto para embeddings de ResNet 128D (0.46 = >99.7% certeza Face ID)
    const UMBRAL_EUCLIDIANO = 0.46;

    if (mejorCoincidencia && menorDistancia <= UMBRAL_EUCLIDIANO) {
      const conf = Math.max(0, Math.min(100, Math.round((1 - (menorDistancia / 0.60)) * 100)));
      return res.json({
        success: true,
        rol: 'ADMINISTRADOR',
        user: {
          id_admin: mejorCoincidencia.id_admin,
          nombre: mejorCoincidencia.admin_nombre,
          usuario: mejorCoincidencia.usuario,
          id_evento: mejorCoincidencia.id_evento,
          nombre_evento: mejorCoincidencia.nombre_evento
        },
        rostro_nombre: mejorCoincidencia.rostro_nombre,
        coincidencia_pct: conf,
        distancia: menorDistancia
      });
    }

    const maxConf = Math.max(0, Math.min(100, Math.round((1 - (menorDistancia / 0.60)) * 100)));
    return res.status(401).json({
      success: false,
      coincidencia_pct: maxConf,
      distancia: menorDistancia,
      message: `Face ID no verificado (${maxConf}% coincidencia). Mira de frente a la cámara.`
    });
  } catch (err) {
    console.error('Error en login facial:', err);
    return res.status(500).json({ success: false, message: 'Error interno en autenticación facial.' });
  }
});

// Gestión de rostros de administrador
app.get('/api/admin/rostros', async (req, res) => {
  try {
    const rostros = await dbAll(
      `SELECT r.id_rostro, r.id_admin, r.nombre, r.foto_referencia, r.fecha_registro, a.nombre AS admin_nombre
       FROM admin_rostro r
       LEFT JOIN administrador_evento a ON a.id_admin = r.id_admin
       ORDER BY r.id_rostro DESC`
    );
    res.json({ success: true, rostros: rostros || [] });
  } catch (err) {
    res.status(500).json({ success: false, message: 'Error al obtener la lista de rostros.' });
  }
});

app.post('/api/admin/rostros', async (req, res) => {
  const { id_admin, nombre, descriptor, foto_referencia } = req.body;
  const nomStr = String(nombre || 'Rostro Admin').trim().slice(0, 100);
  const adminId = parseInt(id_admin, 10) || 1;

  if (!descriptor) {
    return res.status(400).json({ success: false, message: 'Se requiere la firma facial.' });
  }
  ...
  try {
    const resRun = await dbRun(
      `INSERT INTO admin_rostro (id_admin, nombre, descriptor, foto_referencia) VALUES (?, ?, ?, ?)`,
      [adminId, nomStr, descStr, foto_referencia || null]
    );
    registrarAuditoria(adminId, 'REGISTRAR_ROSTRO', 'admin_rostro', resRun.insertId, `Rostro registrado: "${nomStr}"`);
    res.json({ success: true, id_rostro: resRun.insertId, message: `Rostro "${nomStr}" registrado correctamente.` });
  } catch (err) {
    console.error('Error al guardar rostro:', err);
    res.status(500).json({ success: false, message: 'No se pudo guardar el rostro.' });
  }
});

app.delete('/api/admin/rostros/:id', async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id) || id <= 0) {
    return res.status(400).json({ success: false, message: 'ID de rostro no válido.' });
  }
  try {
    await dbRun('DELETE FROM admin_rostro WHERE id_rostro = ?', [id]);
    registrarAuditoria(req.body && req.body.id_admin, 'ELIMINAR_ROSTRO', 'admin_rostro', id, `Se eliminó el rostro #${id}`);
    res.json({ success: true, message: 'Rostro eliminado correctamente.' });
  } catch (err) {
    res.status(500).json({ success: false, message: 'No se pudo eliminar el rostro.' });
  }
});
...
app.get('/api/productos', (req, res) => {
  ...
  const queryProds = `SELECT id_producto, id_categoria, nombre, descripcion, tipo_producto,
                             ...
                             id_botella_vinculada, COALESCE(vasos_por_botella, 10) AS vasos_por_botella,
                             MAX(stock_actual,
...
app.post('/api/comanda', (req, res) => {
  withTransaction(async () => {
    ...
    const productosDb = await dbAll(
      `SELECT id_producto, nombre, id_categoria, precio_venta, stock_actual, activo,
              COALESCE(requiere_acompanante, 0) AS requiere_acompanante,
              COALESCE(es_acompanante, 0) AS es_acompanante,
              id_botella_vinculada, COALESCE(vasos_por_botella, 10) AS vasos_por_botella
       FROM producto
    ...
    for (const [idProd, cantPedida] of cantidadesPorProducto) {
      ...
      if (prod.id_botella_vinculada) {
        ...
        const botellasReq = round4(cantPedida / vasosPorBotella);
    ...
    for (const linea of todasLasLineas) {
      ...
      for (const acomp of linea.acompanantes) {
        const cantAcompTotal = acomp.cantidad * linea.cantidad;
        await dbRun(
          `INSERT INTO detalle_comanda (id_comanda, id_producto, cantidad, precio_unitario, subtotal, id_detalle_padre)
           VALUES (?, ?, ?, 0.00, 0.00, ?)`,
          [idComanda, acomp.id_producto, cantAcompTotal, detPadre.insertId]
        );
      }
    }
    ...
    for (const [idProd, cant] of cantidadesPorProducto) {
      ...
      if (prod.id_botella_vinculada) {
        const vasosPorBotella = Math.max(1, parseInt(prod.vasos_por_botella || 10, 10));
        const botellasADeducir = round4(cant / vasosPorBotella);
        ...
        const nuevoStockBotella = round4(Math.max(0, matchBotella.stock_actual - botellasADeducir));
    ...
    for (const [idPromo, veces] of promosPedidas) {
      ...
      promosResp.push({
        ...
        precio_unitario: Number(pr.precio),
        cantidad: veces,
    ...
    .catch(err => {
      ...
      res.status(500).json({ success: false, message: 'No se pudo guardar la comanda: ' + (err && err.message) });
    });
});
  ...
  Promise.resolve()
    .then(async () => {
      ...
      if (!activa && antes.activa) {
        await registrarAuditoria(req.body.id_admin, 'APAGAR_PROMOCION', 'promocion', id, `"${datos.nombre}" apagada`);
      }
...
app.delete('/api/admin/promociones/:id', (req, res) => {
  ...
  Promise.resolve()
    .then(async () => {
      ...
      const ventas = await dbGet('SELECT COUNT(*) AS n FROM detalle_comanda WHERE id_promocion = ?', [id]);
      const yaVendida = ventas && ventas.n > 0;

      if (yaVendida) {
        await dbRun('UPDATE promocion SET activa = 0, eliminada = 1 WHERE id_promocion = ?', [id]);
        await registrarAuditoria(req.body && req.body.id_admin, 'ELIMINAR_PROMOCION',
          'promocion', id, `"${promo.nombre}" se retiró del catálogo. Como ya se vendió, se conserva para que el historial cuadre.`);
        return res.json({
          success: true,
          retirada: true,
          message: `"${promo.nombre}" se retiró del catálogo. Como ya se vendió, se conserva para que el historial cuadre.`
        });
      }
      ...
      await registrarAuditoria(req.body && req.body.id_admin, 'ELIMINAR_PROMOCION',
        'promocion', id, `"${promo.nombre}" eliminada correctamente.`);
      res.json({ success: true, retirada: false, message: `"${promo.nombre}" eliminada correctamente.` });
    })
    .catch(err => {
...
app.get('/api/admin/catalogo', (req, res) => {
  Promise.all([
    ...
    dbAll(`SELECT p.id_producto, p.nombre, p.precio_venta, p.stock_actual, p.activo, COALESCE(p.orden, 0) AS orden,
                  ...
                  p.id_botella_vinculada, COALESCE(p.vasos_por_botella, 10) AS vasos_por_botella,
...
app.put('/api/admin/productos/:id', (req, res, next) => {
  ...
  const idBotellaVinculada = req.body.id_botella_vinculada ? parseInt(req.body.id_botella_vinculada, 10) : null;
  const vasosPorBotella = Math.max(1, parseInt(req.body.vasos_por_botella || 10, 10));
    ...
    .then(antes => {
      ...
      return dbGet('SELECT id_categoria FROM categoria_producto WHERE id_categoria = ?', [idCategoria])
        .then(cat => {
          ...
          return dbRun(
            'UPDATE producto SET nombre = ?, descripcion = ?, precio_venta = ?, id_categoria = ?, id_botella_vinculada = ?, vasos_por_botella = ? WHERE id_producto = ?',
            [nombre, descripcion, round2(precio), idCategoria, idBotellaVinculada, vasosPorBotella, id]
          ).then(() => {
...
app.get('/api/admin/comandas', (req, res) => {
  const query = `
    SELECT c.*,
           ...
           m.password AS mesero_pin,
           m.usuario AS mesero_usuario,
  ...
  dbQuery(query, (err, comandas) => {
    ...
    dbQuery(queryDetails, (err2, details) => {
      ...
      dbQuery(queryPayments, (err3, payments) => {
        ...
        dbAll('SELECT id_promocion, nombre, precio FROM promocion').then(promosDef => {
          const promoMap = new Map((promosDef || []).map(pr => [pr.id_promocion, pr]));

          const mapped = (comandas || []).map(c => {
            const comDetails = (details || []).filter(d => d.id_comanda === c.id_comanda);
            
            // Reconstruir promociones si existen detalles con id_promocion (Item 5)
            const promosMap = new Map();
            comDetails.forEach(d => {
              if (d.id_promocion && !promosMap.has(d.id_promocion)) {
                const prDef = promoMap.get(d.id_promocion);
                const itemsPromo = comDetails.filter(x => x.id_promocion === d.id_promocion);
                const totalSubPromo = round2(itemsPromo.reduce((sum, it) => sum + Number(it.subtotal || 0), 0));
                const precioUnit = prDef ? Number(prDef.precio) : 0;
                const cantPromo = (precioUnit > 0 && totalSubPromo > 0)
                  ? Math.max(1, Math.round(totalSubPromo / precioUnit))
                  : 1;
                const subtotalPromo = totalSubPromo > 0 ? totalSubPromo : round2(precioUnit * cantPromo);

                promosMap.set(d.id_promocion, {
                  id_promocion: d.id_promocion,
                  nombre: prDef ? prDef.nombre : 'Promoción #' + d.id_promocion,
                  precio: precioUnit,
                  precio_unitario: precioUnit,
                  cantidad: cantPromo,
                  subtotal: subtotalPromo,
                  contenido: itemsPromo.map(it => ({
                    id_producto: it.id_producto,
            ...
            return {
              ...c,
              detalles: comDetails,
              promociones: Array.from(promosMap.values()),
  ...
  withTransaction(async () => {
    ...
    for (const det of detalles) {
      ...
      if (prod && prod.id_botella_vinculada) {
        ...
        const vasosPorBotella = Math.max(1, parseInt(prod.vasos_por_botella || 10, 10));
        const botellasADevolver = round4(det.cantidad / vasosPorBotella);
        if (matchBotella) {
          const nuevoStockBotella = round4(matchBotella.stock_actual + botellasADevolver);
          await dbRun('UPDATE producto SET stock_actual = ? WHERE id_producto = ?', [nuevoStockBotella, matchBotella.id_producto]);
          await dbRun(
            `INSERT INTO movimiento_stock (id_producto, id_admin, tipo_movimiento, cantidad, stock_anterior, stock_nuevo, motivo, fecha_hora)
             VALUES (?, ?, 'ENTRADA', ?, ?, ?, ?, ?)`,
...
app.post('/api/admin/productos', (req, res) => {
  const { id_categoria, nombre, descripcion, tipo_producto, precio_venta, stock_actual, id_admin, id_evento, id_botella_vinculada, vasos_por_botella } = req.body;
  const idBotellaVinculada = id_botella_vinculada ? parseInt(id_botella_vinculada, 10) : null;
  const vasosPorBotella = Math.max(1, parseInt(vasos_por_botella || 10, 10));
  ...
  const query = `INSERT INTO producto (id_categoria, nombre, descripcion, tipo_producto, precio_venta, stock_actual, foto, requiere_acompanante, es_acompanante, orden, creado_por_admin, fecha_creacion, id_botella_vinculada, vasos_por_botella)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, (SELECT COALESCE(MAX(orden), 0) + 1 FROM producto WHERE id_categoria = ?), ?, ?, ?, ?)`;
  dbQuery(query, [id_categoria, String(nombre).trim(), descripcion, tipo_producto, round2(precio), stockInicial, fotoNueva || null, requiere, esAcomp, id_categoria, id_admin, nowStr, idBotellaVinculada, vasosPorBotella], (err, result) => {
...
async function construirReporte(query) {
  ...
  const evento = {
    nombre_evento: config.evento || (sembrado && sembrado.nombre_evento) || 'Serenata a Cochabamba',
    cliente: config.cliente || 'Link',
    fecha_evento: config.fecha || (sembrado && sembrado.fecha_evento) || '13 de Septiembre de 2026',
    lugar: config.lugar || (sembrado && sembrado.lugar) || '',
    responsable: config.responsable || '',
    barra: config.barra || INSTANCIA.nombre || 'VIP'
  };
  ...
  const resumen = await dbGet(`
    SELECT
      ...
      SUM(CASE WHEN estado_pago != 'ANULADO' AND COALESCE(es_cortesia, 0) = 0 THEN 1 ELSE 0 END)              AS validas,
      ...
      SUM(CASE WHEN COALESCE(es_cortesia, 0) = 1 THEN 1 ELSE 0 END)                                           AS cortesias,
      COALESCE(SUM(CASE WHEN COALESCE(es_cortesia, 0) = 1 AND estado_pago != 'ANULADO' THEN total END), 0)               AS importe_cortesia,
      SUM(CASE WHEN COALESCE(es_cortesia, 0) = 1 AND estado_pago != 'ANULADO' THEN 1 ELSE 0 END)             AS cortesias_completadas,
      SUM(CASE WHEN COALESCE(es_cortesia, 0) = 1 AND estado_pago  = 'ANULADO' THEN 1 ELSE 0 END)             AS cortesias_anuladas,
      COALESCE(SUM(CASE WHEN COALESCE(es_cortesia, 0) = 1 AND estado_pago  = 'ANULADO' THEN total END), 0)              AS importe_cortesia_anulado
  ...
  // Desglose general por método de pago
  const porMetodo = await dbAll(`
    ...
    WHERE p.estado = 'APROBADO' AND c.estado_pago != 'ANULADO' AND COALESCE(c.es_cortesia, 0) = 0
  ...
  const porBarra = await dbAll(`
    SELECT b.nombre_barra AS barra,
           COUNT(CASE WHEN c.estado_pago != 'ANULADO' AND COALESCE(c.es_cortesia, 0) = 0 THEN 1 END) AS comandas,
           COALESCE(SUM(CASE WHEN c.estado_pago != 'ANULADO' AND COALESCE(c.es_cortesia, 0) = 0 THEN c.total END), 0) AS importe
    FROM comanda c
    JOIN barra b ON b.id_barra = c.id_barra
  ...
  // Desglose por método de pago por cajero
  const porMetodoCajero = await dbAll(`
    SELECT cj.id_cajero, cj.nombre AS cajero,
           COALESCE(SUM(CASE WHEN UPPER(mp.nombre) = 'EFECTIVO' THEN p.monto ELSE 0 END), 0) AS efectivo,
           COALESCE(SUM(CASE WHEN UPPER(mp.nombre) = 'QR' THEN p.monto ELSE 0 END), 0) AS qr,
           COALESCE(SUM(CASE WHEN UPPER(mp.nombre) = 'TRANSFERENCIA' THEN p.monto ELSE 0 END), 0) AS transferencia,
           COALESCE(SUM(CASE WHEN UPPER(mp.nombre) = 'TARJETA' THEN p.monto ELSE 0 END), 0) AS tarjeta,
    ...
    WHERE p.estado = 'APROBADO' AND c.estado_pago != 'ANULADO' AND COALESCE(c.es_cortesia, 0) = 0
      AND c.fecha_hora BETWEEN ? AND ?
    GROUP BY cj.id_cajero ORDER BY total DESC
  `, P);

  // Desglose por mesero
  const porMeseroMetodo = await dbAll(`
    SELECT m.id_mesero, m.nombre AS mesero,
           COALESCE(SUM(CASE WHEN UPPER(mp.nombre) = 'EFECTIVO' THEN p.monto ELSE 0 END), 0) AS efectivo,
           COALESCE(SUM(CASE WHEN UPPER(mp.nombre) = 'QR' THEN p.monto ELSE 0 END), 0) AS qr,
           COALESCE(SUM(CASE WHEN UPPER(mp.nombre) = 'TRANSFERENCIA' THEN p.monto ELSE 0 END), 0) AS transferencia,
           COALESCE(SUM(CASE WHEN UPPER(mp.nombre) = 'TARJETA' THEN p.monto ELSE 0 END), 0) AS tarjeta,
    ...
    FROM pago_comanda p
    JOIN comanda c ON c.id_comanda = p.id_comanda
    JOIN mesero m ON m.id_mesero = c.id_mesero
    JOIN metodo_pago mp ON mp.id_metodo_pago = p.id_metodo_pago
    WHERE p.estado = 'APROBADO' AND c.estado_pago != 'ANULADO' AND COALESCE(c.es_cortesia, 0) = 0
      AND c.fecha_hora BETWEEN ? AND ?
    GROUP BY m.id_mesero ORDER BY total DESC
  `, P);

  // Movimiento y valorización de productos vendidos
  const productosRaw = await dbAll(`
    ...
    LEFT JOIN promocion pr ON pr.id_promocion = d.id_promocion
    WHERE c.estado_pago != 'ANULADO' AND COALESCE(c.es_cortesia, 0) = 0 AND c.fecha_hora BETWEEN ? AND ?
    GROUP BY p.id_producto, d.precio_unitario, d.id_promocion
    ORDER BY p.nombre ASC
  `, P);

  const productos = productosRaw.map(p => {
    let tipoVenta = 'Venta normal';
    if (p.precio_unitario === 0 || p.es_acompanante) {
      tipoVenta = 'Acompañante incluido';
    } else if (p.nombre_promocion) {
      tipoVenta = p.nombre_promocion;
    }

    let equivalenciaFisica = '-';
    const esVaso = /vaso/i.test(p.producto);
    const esShot = /shot/i.test(p.producto);
    if (esVaso || esShot) {
      const r = esShot ? 10 : 16;
      const botellas = Math.floor(p.cantidad / r);
      const resto = p.cantidad % r;
      const unidad = esShot ? 'shots' : 'vasos';
      if (botellas > 0 && resto > 0) {
        equivalenciaFisica = `${botellas} ${botellas === 1 ? 'botella' : 'botellas'} + ${resto} ${unidad}`;
      } else if (botellas > 0) {
        equivalenciaFisica = `${botellas} ${botellas === 1 ? 'botella' : 'botellas'}`;
      } else {
        equivalenciaFisica = `0 botellas + ${resto} ${unidad}`;
      }
    }
    ...
    return {
      ...
      cantidad: p.cantidad,
      unidades: p.cantidad,
      equivalencia_fisica: equivalenciaFisica,
      tipo_venta: tipoVenta,
      precio_aplicado: round2(p.precio_unitario),
      total: round2(p.total),
      importe: round2(p.total)
    };
  });

  // Cortesías
  const comandasCortesia = await dbAll(`
    SELECT c.id_comanda, c.fecha_hora, c.total, c.estado_pago,
           cj.nombre AS cajero, m.nombre AS mesero
    FROM comanda c
    LEFT JOIN cajero cj ON cj.id_cajero = c.id_cajero
    LEFT JOIN mesero m ON m.id_mesero = c.id_mesero
    WHERE COALESCE(c.es_cortesia, 0) = 1 AND c.fecha_hora BETWEEN ? AND ?
    ORDER BY c.id_comanda ASC
  `, P);

  const detallesCortesia = await dbAll(`
    SELECT d.id_comanda, d.cantidad, p.nombre AS producto
    FROM detalle_comanda d
    JOIN comanda c ON c.id_comanda = d.id_comanda
    JOIN producto p ON p.id_producto = d.id_producto
    WHERE COALESCE(c.es_cortesia, 0) = 1 AND c.fecha_hora BETWEEN ? AND ?
  ...
  const mapDetallesCort = new Map();
  for (const det of detallesCortesia) {
    if (!mapDetallesCort.has(det.id_comanda)) mapDetallesCort.set(det.id_comanda, []);
    mapDetallesCort.get(det.id_comanda).push(`${det.cantidad}x ${det.producto}`);
  }

  const listCortesias = comandasCortesia.map(c => ({
    ...c,
    detalle_texto: (mapDetallesCort.get(c.id_comanda) || []).join('; ')
  }));

  // Detalle de comandas válidas
  const comandasValidas = await dbAll(`
    SELECT c.id_comanda, c.fecha_hora, c.total,
           cj.nombre AS cajero, m.nombre AS mesero
    FROM comanda c
    LEFT JOIN cajero cj ON cj.id_cajero = c.id_cajero
    LEFT JOIN mesero m ON m.id_mesero = c.id_mesero
  ...
  const detallesValidas = await dbAll(`
    SELECT d.id_comanda, d.cantidad, p.nombre AS producto
    FROM detalle_comanda d
    JOIN comanda c ON c.id_comanda = d.id_comanda
    JOIN producto p ON p.id_producto = d.id_producto
  ...
  const mapDetallesVal = new Map();
  for (const det of detallesValidas) {
    if (!mapDetallesVal.has(det.id_comanda)) mapDetallesVal.set(det.id_comanda, []);
    mapDetallesVal.get(det.id_comanda).push(`${det.cantidad}x ${det.producto}`);
  }

  const pagosValidas = await dbAll(`
    SELECT p.id_comanda, mp.nombre AS metodo, p.monto
    FROM pago_comanda p
    JOIN comanda c ON c.id_comanda = p.id_comanda
    JOIN metodo_pago mp ON mp.id_metodo_pago = p.id_metodo_pago
    WHERE c.estado_pago != 'ANULADO' AND COALESCE(c.es_cortesia, 0) = 0 AND c.fecha_hora BETWEEN ? AND ?
  ...
  const mapPagosVal = new Map();
  for (const pg of pagosValidas) {
    if (!mapPagosVal.has(pg.id_comanda)) mapPagosVal.set(pg.id_comanda, []);
    const mName = String(pg.metodo).toUpperCase();
    const abr = mName.startsWith('EF') ? 'Ef.' : mName === 'TARJETA' ? 'Tarj.' : pg.metodo;
    const montoFormatted = Number(pg.monto) % 1 === 0 ? String(Math.round(pg.monto)) : Number(pg.monto).toFixed(2);
    mapPagosVal.get(pg.id_comanda).push(`${abr} ${montoFormatted}`);
  }

  const listComandas = comandasValidas.map(c => ({
    ...c,
    pago_texto: (mapPagosVal.get(c.id_comanda) || []).join(' + '),
    detalle_texto: (mapDetallesVal.get(c.id_comanda) || []).join('; ')
  }));
  ...
  return {
    ...
    resumen: {
      ...
      cortesias_completadas: Number(resumen.cortesias_completadas) || 0,
      cortesias_anuladas: Number(resumen.cortesias_anuladas) || 0,
      ...
      importe_cortesia_anulado: round2(resumen.importe_cortesia_anulado),
...
app.get('/api/admin/reporte.pdf', (req, res) => {
  try {
    delete require.cache[require.resolve('./lib/reporte-cierre')];
    delete require.cache[require.resolve('./lib/pdf')];
  } catch (e) {}
  const { construirPdfCierre, nombreArchivoReporte } = require('./lib/reporte-cierre');
  construirReporte(req.query)
    .then(datos => {
...
try {
  selfsignedModule = require('selfsigned');
} catch (e) {}

async function obtenerCertificadosSSL() {
  if (!selfsignedModule) return null;
  ...
  if (fs.existsSync(certPath) && fs.existsSync(keyPath)) {
    try {
      ...
      if (cert && cert.includes('BEGIN CERTIFICATE') && key && key.includes('KEY')) {
        return { cert, key };
      }
    } catch (e) {}
  }
  ...
  try {
    const attrs = [{ name: 'commonName', value: 'MasterDrinks-Local' }];
    ...
    console.warn('No se pudo generar certificado SSL automático:', errGen.message);
  }
  return null;
}
...
async function iniciarServidor() {
  ...
  let sslCreds = null;
  try {
    sslCreds = await obtenerCertificadosSSL();
  } catch (e) {}

  const HTTPS_PORT = Number(process.env.HTTPS_PORT) || 3443;

  // Servidor HTTP Principal (Siempre disponible para red local y tests)
  const servidorHttp = app.listen(PORT, '0.0.0.0', () => {
    const ips = localAddresses();
    const rotulo = INSTANCIA.nombre.toUpperCase();
    console.log(`\n==================================================`);
    console.log(`   B A R R A :   ${rotulo}`);
    console.log(`==================================================`);
    console.log(`🚀 MasterDrinks POS iniciado (HTTP en puerto ${PORT})`);
...
`);
  ...
  // Servidor HTTPS Secundario si hay certificados
  if (sslCreds && !process.env.NO_SSL) {
    try {
      const servidorHttps = https.createServer(sslCreds, app);
      servidorHttps.listen(HTTPS_PORT, '0.0.0.0', () => {
        // En segundo plano
      });
      servidorHttps.on('error', err => {
        if (err.code !== 'EADDRINUSE') console.warn('Advertencia puerto HTTPS:', err.message);
      });
    } catch (e) {}
  }
}
