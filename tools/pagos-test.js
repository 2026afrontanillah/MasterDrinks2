...
(async () => {
  ...
  dbInit.exec("INSERT OR REPLACE INTO cajero (id_cajero, id_barra, nombre, usuario, password, activo) VALUES (1, (SELECT id_barra FROM barra LIMIT 1), 'Ana Torres', 'cajero_norte_1', 'demo123', 1)");
  dbInit.exec("INSERT OR REPLACE INTO mesero (id_mesero, id_evento, id_cajero, nombre, usuario, password, activo) VALUES (9, 1, 1, 'Mesero Test', 'mesero_09', '1009', 1)");
  dbInit.close();
  ...
  // ---- Tras cobrar: se abre la vista previa del ticket ----
  check('Se abre la ventana de vista previa del ticket',
    !t.id('print-modal').classList.contains('hide'), 'modal de vista previa visible');

  // Al pulsar Continuar o cerrar la vista previa, vuelve a la pantalla del PIN
  t.click(t.id('dismiss-print-btn'));
  await esperar(120);

  check('La tablet vuelve a la pantalla del PIN',
