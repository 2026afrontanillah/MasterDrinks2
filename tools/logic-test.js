...
(async () => {
  ...
  const prod = productos.find(p => !p.requiere_acompanante && !p.es_acompanante && p.stock_actual >= 20)
    || productos.find(p => p.stock_actual >= 20)
    || productos.find(p => !p.requiere_acompanante && !p.es_acompanante && p.stock_actual > 0)
    || productos[0];
  ...
  const idCajero = (leer('SELECT id_cajero FROM cajero WHERE activo = 1 ORDER BY id_cajero')[0] || { id_cajero: 1 }).id_cajero;
  const idMesero = (leer('SELECT id_mesero FROM mesero WHERE activo = 1 ORDER BY id_mesero')[0] || { id_mesero: 1 }).id_mesero;
  const base = {
    id_evento: 1, id_barra: idBarra, id_cajero: idCajero, id_mesero: idMesero,
  ...
  const meseroOtraCaja = leer('SELECT id_mesero, password FROM mesero WHERE id_cajero != 1 AND activo = 1 LIMIT 1')[0]
    || leer('SELECT id_mesero, password FROM mesero WHERE activo = 1 LIMIT 1')[0];
  if (meseroOtraCaja) {
    r = await post('/api/login/mesero', { password: meseroOtraCaja.password, id_cajero: 1, id_barra: idBarra, id_evento: 1 });
    check('Acepta el PIN de un mesero de otra caja del mismo servidor',
      r.status === 200 && r.json.mesero.id_mesero === meseroOtraCaja.id_mesero);
  }

  const mesero1 = leer('SELECT password FROM mesero WHERE activo = 1 LIMIT 1')[0];
  r = await post('/api/admin/meseros', {
    id_cajero: 3, nombre: 'Duplicado', usuario: 'mesero_dup_test',
    password: mesero1 ? mesero1.password : '1001', id_admin: 1, id_evento: 1
  });
  ...
  const recaudadoEnBase = leer("SELECT COALESCE(SUM(total),0) t FROM comanda WHERE estado_pago != 'ANULADO' AND COALESCE(es_cortesia, 0) = 0")[0].t;
  ...
  check('Se descarga como archivo con la barra en el nombre',
    new RegExp('attachment; filename="(?:Cierre|Reporte_Barra)_' + barraEnNombre + '_.*\\.pdf"')
  ...
  check('El PDF dice a qué barra corresponde',
    texto.includes(identAhora.nombre) || texto.includes(identAhora.nombre.toUpperCase()));
  ...
  check('El PDF nombra el evento y las secciones del cierre',
    (texto.includes('Reporte') || texto.includes('MASTERDRINKS')) &&
    (texto.includes('cajero') || texto.includes('Cajero') || texto.includes('CAJERO')));
  ...
  let libre = leer(`SELECT p.id_producto, p.nombre FROM producto p
                      WHERE p.activo = 1
                        AND NOT EXISTS (SELECT 1 FROM detalle_comanda d WHERE d.id_producto = p.id_producto)
                        AND NOT EXISTS (SELECT 1 FROM traspaso_detalle t WHERE t.id_producto = p.id_producto)
                      LIMIT 1`)[0];
  if (!libre) {
    const prodNuevo = await post('/api/admin/productos', {
      nombre: 'Producto Libre Test ' + Date.now(),
      id_categoria: 1, precio_venta: 10, stock_inicial: 5, id_admin: 1
    });
    libre = { id_producto: prodNuevo.json.producto.id_producto, nombre: prodNuevo.json.producto.nombre };
  }
  ...
  let mesLibre = leer(`SELECT m.id_mesero FROM mesero m
                         WHERE m.activo = 1
                           AND NOT EXISTS (SELECT 1 FROM comanda k WHERE k.id_mesero = m.id_mesero)
                         LIMIT 1`)[0];
  if (!mesLibre) {
    const nuevoM = await post('/api/admin/meseros', {
      id_cajero: 1, nombre: 'Mesero Lib ' + Date.now(), usuario: 'mes_lib_' + Date.now(),
      password: '98' + (Math.floor(Math.random() * 80) + 10), id_admin: 1, id_evento: 1
    });
    mesLibre = { id_mesero: nuevoM.json && (nuevoM.json.id_mesero || (nuevoM.json.mesero && nuevoM.json.mesero.id_mesero)) };
  }
