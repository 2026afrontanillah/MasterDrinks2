...
(async () => {
  ...
  if (arriba) {
    ...
    sem.exec("INSERT OR REPLACE INTO cajero (id_cajero, id_barra, nombre, usuario, password, activo) VALUES (1, (SELECT id_barra FROM barra LIMIT 1), 'Ana Torres', 'cajero_norte_1', 'demo123', 1)");
    sem.exec("INSERT OR REPLACE INTO mesero (id_mesero, id_evento, id_cajero, nombre, usuario, password, activo) VALUES (9, 1, 1, 'Mesero Test', 'mesero_09', '1009', 1)");
    sem.close();
  }
  if (!arriba) {
  ...
  const idSeg = segunda.dataset.id;
  click(window.document.querySelector(`.product-card[data-id="${idPrimera}"]`));
  click(window.document.querySelector(`.product-card[data-id="${idPrimera}"]`));
  click(window.document.querySelector(`.product-card[data-id="${idSeg}"]`));
  await esperar(120);
  check('Y se puede volver a montar el pedido', id('cart-count').textContent === '2', id('cart-count').textContent);
...
// =======================================================================
  ...
  check('El combo sale en la rejilla, con su sello', !!promoCard &&
    (!!promoCard.querySelector('.promo-sello') || !!promoCard.querySelector('.promo-badge-tag')),
    promoCard ? (promoCard.querySelector('h3') || promoCard.querySelector('.product-card-title') || {}).textContent : 'no está');
  check('Y enseña lo que lleva dentro, sin tener que abrirlo',
    promoCard && (promoCard.querySelectorAll('.promo-dentro li').length >= 2 || (promoCard.querySelector('.promo-dentro') && promoCard.querySelector('.promo-dentro').textContent.includes('×')) || (promoCard.querySelector('.promo-items-text') && promoCard.querySelector('.promo-items-text').textContent.includes('×'))),
    promoCard ? (promoCard.querySelector('.promo-dentro') || promoCard.querySelector('.promo-items-text') || {}).textContent : '');
  ...
  check('Y debajo, lo que hay que servir',
    lineaPromo && (lineaPromo.querySelectorAll('.cart-acomp').length >= 1 || lineaPromo.textContent.includes('×') || lineaPromo.textContent.includes('+') || lineaPromo.textContent.includes('Ron')),
    lineaPromo ? lineaPromo.textContent.trim() : '');
  ...
  check('Tras volver a entrar, el carrito se rehace',
    id('cart-count').textContent.length > 0 && id('cart-count').textContent !== '0', id('cart-count').textContent);
  ...
  check('La ✕ limpia y devuelve el catálogo entero',
    ...
    window.document.querySelectorAll('.product-card[data-id]').length > 0,
    window.document.querySelectorAll('.product-card[data-id]').length + ' de ' + totalTarjetas);
  check('El carrito sobrevive al filtrado', id('cart-count').textContent.length > 0 && id('cart-count').textContent !== '0');
  ...
  check('El ticket imprime el precio unitario', /\d+\.\d\d/.test(previa));
  ...
  check('El ticket de barra muestra los productos a despachar', /x\d+/.test(previaMesero));
  ...
  id('login-password').value = 'admin*12345';
  $('#login-form').dispatchEvent(new window.Event('submit', { bubbles: true, cancelable: true }));
  await esperar(500);
  ...
  // Esperar a que la lista de personal termine de cargar
  for (let i = 0; i < 20; i++) {
    await esperar(100);
    if (window.document.querySelectorAll('#lista-cajeros .lista-fila').length > 0) break;
  }
  await esperar(200);
  ...
  const enAdmin = await (await fetch(URL_BASE + '/api/admin/catalogo')).json();
  const yaCambiado = (enAdmin.productos || [])
