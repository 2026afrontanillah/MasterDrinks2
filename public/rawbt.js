...
window.ThermalPrinter = (function () {
  ...
  const DEFAULTS = {
    ...
    feed: 0,            // corte ajustado sin desperdicio de papel
    autoPrint: true,
    showPreview: false  // false = impresión directa sin modal de vista previa al cobrar
  };
  ...
  function buildCajeroOps(model, settings) {
    ...
    // Cabecera de columnas para el ticket de cobro
    const wCant = w >= 40 ? 8 : 7;
    const wPrecio = w >= 40 ? 8 : 7;
    const wTotal = w >= 40 ? 9 : 7;
    const wNombre = Math.max(10, w - wCant - wPrecio - wTotal);

    ops.push(op(
      'CANT.'.padEnd(wCant) +
      'PRODUCTO'.padEnd(wNombre) +
      'PRECIO'.padStart(wPrecio) +
    ...
    function emitirFilaCajero(cant, nombre, precio, total, isChild, isPromoChild, isPromoParent) {
      ...
      if (isPromoParent) {
        textoNombre = /^promoci[oó]n|^promo/i.test(nombre) ? nombre : ('PROMO ' + nombre);
      }
      const trozos = wrap(textoNombre, remNameWidth);

      trozos.forEach((l, i) => {
        if (i === 0) {
          ops.push(op(cantCol + l.padEnd(remNameWidth) + precioStr.padStart(wPrecio) + totalStr.padStart(wTotal)));
        } else {
          ops.push(op(' '.repeat(wCant) + l));
        }
      });
    }

    (model.promociones || []).forEach((pr, idx) => {
      if (idx > 0) ops.push(op(''));
      emitirFilaCajero(pr.cantidad, pr.nombre, pr.precio_unitario, pr.subtotal, false, false, true);
      (pr.contenido || []).forEach(c => {
        emitirFilaCajero(c.cantidad, 'ENTREGAR: ' + c.cantidad + ' ' + (c.nombre || ''), null, null, true, true, false);
      });
    });

    const tienePromos = (model.promociones || []).length > 0;
    (model.items || []).forEach((item, idx) => {
      if (idx > 0 || tienePromos) ops.push(op(''));
      const unitario = item.precio_unitario != null
        ? Number(item.precio_unitario)
        : Number(item.subtotal || 0) / Math.max(1, Number(item.cantidad || 1));
      emitirFilaCajero(item.cantidad, item.nombre, unitario, item.subtotal, false, false, false);

      (item.acompanantes || []).forEach(a => {
        emitirFilaCajero(a.cantidad, a.nombre, null, null, true, false, false);
      });
    });
  ...
  /** Ticket de preparación (copia del mesero, con precios pero sin total a pagar). */
  function buildMeseroOps(model, settings) {
    ...
    const wPrecioM = w >= 40 ? 8 : 7;
    const wTotalM = w >= 40 ? 9 : 7;
    const wNombreM = Math.max(10, w - wCantM - wPrecioM - wTotalM);

    ops.push(op(
      'CANT.'.padEnd(wCantM) +
      'PRODUCTO'.padEnd(wNombreM) +
      'PRECIO'.padStart(wPrecioM) +
      'TOTAL'.padStart(wTotalM)
    , { bold: true }));
    ops.push(op(divider(w)));
    ...
    function emitirFilaMesero(cant, nombre, precio, total, isChild, isPromoChild, isPromoParent) {
      ...
      trozos.forEach((l, i) => {
        if (i === 0) {
          ops.push(op(cantCol + l.padEnd(remNameWidth) + precioStr.padStart(wPrecioM) + totalStr.padStart(wTotalM), { bold: !isChild || isPromoChild }));
        } else {
          ops.push(op(' '.repeat(wCantM) + l, { bold: !isChild || isPromoChild }));
        }
      });
    }
    ...
    (model.promociones || []).forEach((pr, idx) => {
      if (idx > 0) ops.push(op(''));
      emitirFilaMesero(pr.cantidad, pr.nombre, pr.precio_unitario, pr.subtotal, false, false, true);
      (pr.contenido || []).forEach(c => {
        emitirFilaMesero(c.cantidad, 'ENTREGAR: ' + c.cantidad + ' ' + (c.nombre || ''), null, null, true, true, false);
      });
    });

    const tienePromosM = (model.promociones || []).length > 0;
    (model.items || []).forEach((item, idx) => {
      if (idx > 0 || tienePromosM) ops.push(op(''));
      const unitario = item.precio_unitario != null
        ? Number(item.precio_unitario)
        : Number(item.subtotal || 0) / Math.max(1, Number(item.cantidad || 1));
      emitirFilaMesero(item.cantidad, item.nombre, unitario, item.subtotal, false, false, false);

      (item.acompanantes || []).forEach(a => {
        emitirFilaMesero(a.cantidad, a.nombre, null, null, true, false, false);
      });
    });
  ...
  function opsToEscPos(ops, settings) {
    ...
    if (settings.feed && Number(settings.feed) > 0) {
      push(ESC, 0x64, Math.max(0, settings.feed | 0));   // ESC d n — avanzar papel si se requiere
    }
    if (settings.cut) push(GS, 0x56, 0x42, 0x00);      // GS V B 0 — corte parcial ajustado
  ...
  function buildTraspasoOps(model, settings) {
    ...
    if (settings.printLogo && logoTicketUrl) {
      ops.push(op('LOGO', { isLogo: true, align: 'center' }));
    }
    ...
    if (model.responsable) kv(saliendo ? 'Entrega' : 'Recibe', model.responsable, w).forEach(l => ops.push(op(l)));
  ...
  function printOpsViaBrowser(ops, settings, titulo) {
    settings = settings || getSettings();
    const maxMm = settings.width >= 48 ? '78mm' : '58mm';
    const html = opsToHtml(ops, settings);
  ...
  return {
    ...
    printOpsViaBrowser: printOpsViaBrowser,
