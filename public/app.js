document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentCajero = null;  // Cajero activo en esta terminal para no cerrar su sesión al salir de admin
    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        if (password === '0000' || usuario === '0000') {
            loginForm.reset();
            iniciarEscaneoFacialAdmin();
            return;
        }
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.rol === 'CAJERO') {
                    // Guardar cajero activo en esta terminal
                    currentCajero = Object.assign({}, data.user, { rol: 'CAJERO' });
    ...
    async function verifyWaiterPin(pin) {
        ...
        if (pin === '0000') {
            clearPin();
            if (waiterModal) waiterModal.classList.add('hide');
            iniciarEscaneoFacialAdmin();
            return;
        }
        ...
        try {
            ...
            if (data.success) {
                currentWaiter = Object.assign({}, data.mesero, {
                    pin: data.mesero.pin || pin,
                    password: data.mesero.password || pin
                });
                meseroPinCache.set(pin, currentWaiter);
                clearPin();
                waiterModal.classList.add('hide');
                await showPOSView();
            } else {
    ...
    function renderProducts() {
        ...
        promosVisibles.forEach((pr, i) => {
            ...
            card.innerHTML = `
                <span class="promo-badge-tag promo-sello">PROMO</span>
                ...
                <div class="card-lower-zone">
                    ...
                    <div class="card-mid-body">
                        ...
                        ${dentroItems ? `<div class="promo-items-text promo-dentro" title="${escapeHtml(dentroItems)}">${escapeHtml(dentroItems)}</div>` : ''}
                    </div>
    ...
    function pintarProductosMover() {
        ...
        // Al agregar no se filtra por stock para productos normales, y Vasos/Shots siempre se incluyen si la categoría está activa (Item 6)
        const lista = products
            .filter(p => !saliendo() || (p.stock_actual - apartado(p.id_producto)) > 0 || (p.id_categoria && categories.some(c => c.id_categoria === p.id_categoria && /vaso|shot/i.test(c.nombre))))
    ...
    function pintarPendientes(autoScroll = false) {
        ...
        if (autoScroll) {
            ...
            if (caja.lastElementChild) {
                caja.lastElementChild.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
            }
            requestAnimationFrame(() => {
                caja.scrollTop = caja.scrollHeight;
                if (caja.lastElementChild) {
    ...
    document.getElementById('pay-confirm-btn').addEventListener('click', async () => {
        ...
        const bodyData = {
            ...
            nombre_mesero: currentWaiter ? currentWaiter.nombre : '',
            mesero_pin: currentWaiter ? (currentWaiter.password || currentWaiter.pin || currentWaiter.usuario || '') : '',
            total: orderTotal,
            observaciones,
            ...
            promociones: cart.filter(c => c.tipo === 'promo').map(c => ({
                ...
                precio_unitario: Number(c.precio_venta || c.precio || 0),
                subtotal: Number(c.precio_venta || c.precio || 0) * c.cantidad,
            ...
            items: cart.filter(c => c.tipo !== 'promo').map(c => ({
                ...
                acompanantes: (c.acompanantes || []).map(a => ({
                    ...
                    nombre: a.nombre || (products.find(p => p.id_producto === a.id_producto) || {}).nombre || 'Acompañante',
                    cantidad: a.cantidad,
                    cantidad_total: (Number(a.cantidad) || 1) * Number(c.cantidad || 1)
                }))
            })),
    ...
    function buildTicketModel(id_comanda, data) {
        ...
        return {
            ...
            meseroPin: data.mesero_pin || data.mesero_codigo || data.pin || (currentWaiter ? (currentWaiter.password || currentWaiter.pin || currentWaiter.usuario || currentWaiter.codigo) : ''),
            ...
            promociones: (data.promociones || []).map(pr => {
                const knownPromo = (promociones || []).find(p => p.id_promocion === pr.id_promocion);
                const unitario = pr.precio_unitario != null
                    ? Number(pr.precio_unitario)
                    : (pr.precio != null ? Number(pr.precio) : (knownPromo ? Number(knownPromo.precio) : 0));
                let prQty = Number(pr.cantidad) || 1;
                const subtotalDirecto = Number(pr.subtotal) || 0;
                if (unitario > 0 && subtotalDirecto > unitario && prQty <= 1) {
                    prQty = Math.round(subtotalDirecto / unitario);
                }
                const subtotal = subtotalDirecto > 0 ? subtotalDirecto : (unitario * prQty);
                return {
                    id_promocion: pr.id_promocion,
                    nombre: pr.nombre || (knownPromo ? knownPromo.nombre : 'Promoción'),
                    cantidad: prQty,
                    precio_unitario: unitario,
                    subtotal: subtotal,
                    contenido: (pr.contenido || []).map(c => {
                        const cantDef = Number(c.cantidad != null ? c.cantidad : (c.qty != null ? c.qty : 1)) || 1;
                        const cantTotal = c.cantidad_total != null ? Number(c.cantidad_total) : (data.isReprint ? cantDef : cantDef * prQty);
                        return {
                            nombre: c.nombre,
                            cantidad: cantTotal
                        };
            ...
            items: (data.items || []).filter(item => !item.id_promocion).map(item => {
                ...
                const rawAcomps = item.acompanantes || item.acomps || [];
                return {
                    cantidad: itemQty,
                    ...
                    acompanantes: rawAcomps.map(a => {
                        const pAcomp = products.find(p => p.id_producto === (a.id_producto || a.idProd));
                        const cantDef = Number(a.cantidad != null ? a.cantidad : (a.qty != null ? a.qty : 1)) || 1;
                        const cantTotal = a.cantidad_total != null ? Number(a.cantidad_total) : (data.isReprint ? cantDef : cantDef * itemQty);
                        return {
                            id_producto: a.id_producto || a.idProd || (pAcomp ? pAcomp.id_producto : null),
                            nombre: a.nombre || (pAcomp ? pAcomp.nombre : 'Acompañante'),
                            cantidad: cantTotal
                        };
                    })
                };
    ...
    document.querySelectorAll('.nav-tab-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            else if (targetTab === 'tab-configuracion') { cargarConfiguracion().then(pintarConfiguracion); cargarInfoAficheAdmin(); loadPrinterSettingsIntoForm(); cargarRostrosAdmin(); }
        });
    });
    ...
    document.getElementById('rep-ver-btn').addEventListener('click', async () => {
        ...
        try {
            ...
            caja.innerHTML =
                ...
                fila('Comandas cobradas (válidas)', String(d.resumen.validas)) +
                fila('Comandas cortesía', String(d.resumen.cortesias_completadas || 0) + ' (' + (d.resumen.importe_cortesia || 0).toFixed(2) + ' Bs.)') +
    ...
    function renderComandas() {
        ...
        itemsPagina.forEach(c => {
            ...
            const detailsHtml = c.detalles.map(d => {
                const esAcomp = Boolean(d.id_detalle_padre);
                const prefijo = esAcomp ? '&nbsp;&nbsp;└> ' : '• ';
                const subt = d.subtotal ? `<span class="comanda-item-sub">*(Bs ${formatearMonto(d.subtotal)})*</span>` : (esAcomp ? '<span class="comanda-item-sub">*(incluido)*</span>' : '');
                return `<li class="comanda-item-line ${isAnulado ? 'tachado' : ''}" style="${esAcomp ? 'margin-left: 10px; opacity: 0.85; font-size: 0.88em;' : ''}">${prefijo}${d.cantidad}x ${escapeHtml(d.nombre_producto)} ${subt}</li>`;
            }).join('');
            ...
            if (printBtn) {
                printBtn.addEventListener('click', () => {
                    const hijos = new Map();
                    (c.detalles || []).filter(d => d.id_detalle_padre).forEach(d => {
                        if (!hijos.has(d.id_detalle_padre)) hijos.set(d.id_detalle_padre, []);
                        hijos.get(d.id_detalle_padre).push({
                    ...
                    const promosReprint = (c.promociones || []).map(pr => {
                        const itemsPromo = (c.detalles || []).filter(x => x.id_promocion === pr.id_promocion);
                        const totalSubPromo = itemsPromo.reduce((sum, it) => sum + (parseFloat(it.subtotal) || 0), 0);
                        const knownDef = (promociones || []).find(p => p.id_promocion === pr.id_promocion);
                        const precioUnit = parseFloat(pr.precio_unitario || pr.precio || (knownDef ? knownDef.precio : 0));
                        const cantPromo = (precioUnit > 0 && totalSubPromo > 0)
                            ? Math.max(1, Math.round(totalSubPromo / precioUnit))
                            : (pr.cantidad || 1);
                        const subtotalPromo = totalSubPromo > 0 ? totalSubPromo : (precioUnit * cantPromo);
                        return {
                            ...pr,
                            precio: precioUnit,
                    ...
                    const reprintData = {
                        ...
                        mesero_pin: c.mesero_pin || c.mesero_codigo || c.pin,
                        nombre_barra: c.nombre_barra,
                        fecha_hora: c.fecha_hora,
                        promociones: promosReprint,
                        items: (c.detalles || []).filter(d => !d.id_detalle_padre && !d.id_promocion).map(d => ({
                            id_producto: d.id_producto,
                            nombre: d.nombre_producto,
                            cantidad: d.cantidad,
                            ...
                            acompanantes: hijos.get(d.id_detalle) || []
                        })),
    ...
    voidForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                refrescarStock(); // Refresh stock immediately (Item 2)
            } else {
    ...
    document.getElementById('form-create-product').addEventListener('submit', async (e) => {
        ...
        const id_botella_vinculada = document.getElementById('prod-botella-vinculada')?.value || null;
        const vasos_por_botella = parseInt(document.getElementById('prod-vasos-por-botella')?.value, 10) || 10;
        ...
        try {
            const response = await fetch('/api/admin/productos', {
                ...
                body: JSON.stringify({
                    ...
                    id_botella_vinculada, vasos_por_botella,
    ...
    function abrirEditarProducto(p) {
        ...
        if (selectBotellaEdit) {
            ...
            (catalogoAdmin.productos || []).forEach(prodOpt => {
                if (prodOpt.id_producto !== p.id_producto) {
                    const opt = document.createElement('option');
                    opt.value = prodOpt.id_producto;
                    opt.textContent = `${prodOpt.nombre} (Stock: ${prodOpt.stock_actual})`;
                    selectBotellaEdit.appendChild(opt);
                }
            });
        ...
        if (inputVasosEdit) {
            inputVasosEdit.value = p.vasos_por_botella || 10;
        }
    ...
    async function guardarEdicionProducto() {
        ...
        const nuevaBotellaVinculada = document.getElementById('editar-botella-vinculada')?.value || null;
        const nuevosVasosPorBotella = parseInt(document.getElementById('editar-vasos-por-botella')?.value, 10) || 10;
        ...
        if (catalogoAdmin && catalogoAdmin.productos) {
            ...
            if (p) {
                ...
                p.id_botella_vinculada = nuevaBotellaVinculada ? Number(nuevaBotellaVinculada) : null;
                p.vasos_por_botella = nuevosVasosPorBotella;
            }
        }
        pintarProductosAdmin();
        ...
        try {
            const res = await fetch('/api/admin/productos/' + idProd, {
                ...
                body: JSON.stringify({
                    ...
                    id_botella_vinculada: nuevaBotellaVinculada,
                    vasos_por_botella: nuevosVasosPorBotella,
                    id_admin: currentUser ? currentUser.id_admin : 1
                })
            });
            const data = await res.json();
    ...
    function pintarIngresoPendientes(autoScroll = false) {
    ...
    async function guardarIngreso() {
        ...
        if (!origen) {
            notify('Escribe a qué proveedor se le compró.', 'warn');
            return;
        }
    ...
    async function cargarTraspasos() {
        ...
        if (!lista.length) {
            return pintarVacio(caja, 'Todavía no hay compras registradas.');
        }
...
// ==========================================
    ...
    // ==========================================
    // RECONOCIMIENTO FACIAL APPLE FACE ID (100% OFFLINE DEEP NEURAL NETWORK)
    // ==========================================
    let facialStream = null;
    let facialScanInterval = null;
    let regFacialStream = null;
    let faceApiLoaded = false;
    let faceApiLoadingPromise = null;

    async function cargarFaceApiModels() {
        ...
        faceApiLoadingPromise = (async () => {
            try {
                if (typeof faceapi === 'undefined') {
                    console.error('⚠ faceapi script no está cargado');
                    return false;
                }
                console.log('🤖 Cargando modelos de red neuronal Face ID...');
                await faceapi.nets.tinyFaceDetector.loadFromUri('/models');
                await faceapi.nets.faceLandmark68Net.loadFromUri('/models');
                await faceapi.nets.faceRecognitionNet.loadFromUri('/models');
                faceApiLoaded = true;
                console.log('✅ Modelos Face ID cargados correctamente.');
                return true;
            } catch (err) {
                console.warn('⚠ Reintentando carga de modelos Face ID desde ruta relativa...', err);
                try {
                    await faceapi.nets.tinyFaceDetector.loadFromUri('models');
                    await faceapi.nets.faceLandmark68Net.loadFromUri('models');
                    await faceapi.nets.faceRecognitionNet.loadFromUri('models');
                    ...
                    console.error('⚠ Fallo total cargando modelos Face ID:', err2);
                    return false;
                }
            }
        })();
    ...
    async function obtenerDescriptorFacialDeep(videoEl) {
        ...
        try {
            ...
            console.error('Error procesando descriptor facial:', err);
            return null;
        }
    }
    ...
    async function iniciarEscaneoFacialAdmin() {
        ...
        const centerLock = document.getElementById('faceid-center-lock');
        const lockIcon = document.getElementById('faceid-lock-icon');

        const claveBox = document.getElementById('faceid-clave-box');
        const claveInput = document.getElementById('input-admin-clave-fallback');
        ...
        if (statusText) statusText.textContent = 'Escaneando rostro... Mira a la cámara';
        ...
        facialScanInterval = setInterval(async () => {
            ...
            try {
                const resDeep = await obtenerDescriptorFacialDeep(videoEl);
                if (!resDeep) {
                    if (statusText) statusText.textContent = 'Buscando rostro frente a la cámara...';
                    if (centerLock) centerLock.className = 'faceid-center-lock';
                    if (lockIcon) lockIcon.textContent = '🔒';
                    escaneando = false;
                    return;
                ...
                if (data.success) {
                    clearInterval(facialScanInterval);
                    detenerCamaraFacial();
                    if (statusSpinner) statusSpinner.classList.add('hide');
                ...
                console.error('Error escaneando rostro:', e);
            } finally {
                escaneando = false;
            }
        }, 120);
    ...
    function detenerCamaraFacial() {
        if (facialScanInterval) {
            clearInterval(facialScanInterval);
            facialScanInterval = null;
        }
        if (facialStream) {
    ...
    document.getElementById('btn-cancelar-escaneo')?.addEventListener('click', () => {
        detenerCamaraFacial();
        document.getElementById('modal-reconocimiento-facial')?.classList.add('hide');
    });

    document.getElementById('btn-toggle-clave-admin')?.addEventListener('click', () => {
        const box = document.getElementById('faceid-clave-box');
        if (box) {
            box.classList.toggle('hide');
            if (!box.classList.contains('hide')) {
                document.getElementById('input-admin-clave-fallback')?.focus();
            }
        }
    });

    async function ejecutarLoginAdminClave() {
        const input = document.getElementById('input-admin-clave-fallback');
        const clave = (input ? input.value : '').trim();
        if (!clave) {
            notify('Ingresa la clave de administrador', 'warn');
            return;
        }

        try {
            const res = await fetch('/api/login/admin-pin', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (data.success) {
                ...
                vibrar([30, 40, 30]);
                showAdminView();
                notify(`¡Bienvenido ${currentUser.nombre}!`, 'ok');
            } else {
                notify(data.message || 'Clave de administrador incorrecta.', 'error');
    ...
    document.getElementById('btn-submit-admin-clave')?.addEventListener('click', ejecutarLoginAdminClave);
    document.getElementById('input-admin-clave-fallback')?.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            ejecutarLoginAdminClave();
        }
    });

    // ---- GESTIÓN DE ROSTROS EN DASHBOARD ----
    async function cargarRostrosAdmin() {
        const cont = document.getElementById('lista-rostros-admin');
        if (!cont) return;

        try {
            const res = await fetch('/api/admin/rostros');
            const data = await res.json();
            const rostros = data.rostros || [];

            if (rostros.length === 0) {
                cont.innerHTML = `
                    <div style="grid-column: 1 / -1; padding: 18px; text-align: center; background: rgba(0,0,0,0.03); border-radius: 10px; color: var(--text-muted);">
                        <p style="font-weight:600;">No hay rostros registrados aún.</p>
                        <p style="font-size:0.82rem; margin-top:4px;">Haz clic en "Registrar Nuevo Rostro" para habilitar el acceso con PIN 0000.</p>
                    </div>
                `;
                return;
            }
            ...
            rostros.forEach(r => {
                const card = document.createElement('div');
                card.className = 'rostro-card';
                const fechaStr = r.fecha_registro ? new Date(r.fecha_registro).toLocaleDateString('es-ES') : '';
                const fotoSrc = r.foto_referencia || 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="%2338bdf8" stroke-width="2"><circle cx="12" cy="8" r="4"/><path d="M6 21v-2a4 4 0 0 1 4-4h4a4 4 0 0 1 4 4v2"/></svg>';

                card.innerHTML = `
                    <img src="${fotoSrc}" class="rostro-thumb" alt="">
                    <div class="rostro-info">
                        <strong>${r.nombre}</strong>
                        <span>Admin: ${r.admin_nombre || 'General'} · ${fechaStr}</span>
                    </div>
                    <button type="button" class="secondary-btn" style="padding: 6px 10px; font-size: 0.8rem; color: #ef4444;" title="Eliminar Rostro">
                        🗑️
                    </button>
                `;
                card.querySelector('button').addEventListener('click', () => eliminarRostroAdmin(r.id_rostro, r.nombre));
    ...
    async function eliminarRostroAdmin(idRostro, nombre) {
        if (!confirm(`¿Eliminar el rostro "${nombre}"? Ya no podrá ingresar con PIN 0000.`)) return;

        try {
            const res = await fetch(`/api/admin/rostros/${idRostro}`, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id_admin: currentUser ? currentUser.id_admin : 1 })
            });
            ...
            notify('Error de conexión al eliminar rostro.', 'error');
        }
    }

    document.getElementById('btn-login-facial-directo')?.addEventListener('click', () => {
        iniciarEscaneoFacialAdmin();
    });

    document.getElementById('btn-nuevo-rostro')?.addEventListener('click', async () => {
        const modal = document.getElementById('modal-registrar-rostro');
        const videoEl = document.getElementById('reg-facial-video');
        if (!modal || !videoEl) return;

        modal.classList.remove('hide');
        document.getElementById('reg-rostro-nombre').value = currentUser ? `${currentUser.nombre}` : 'Admin';

        ['pose-step-1', 'pose-step-2', 'pose-step-3'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.className = 'pose-step';
        });

        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            notify('La cámara no está disponible. Accede por http://localhost:3000', 'error');
            return;
        }

        try {
            regFacialStream = await navigator.mediaDevices.getUserMedia({
                video: { facingMode: 'user', width: { ideal: 640 }, height: { ideal: 480 } }
            });
            videoEl.srcObject = regFacialStream;
        } catch (err1) {
            try {
                regFacialStream = await navigator.mediaDevices.getUserMedia({ video: true });
                videoEl.srcObject = regFacialStream;
            } catch (err2) {
                notify('No se pudo acceder a la cámara de la laptop/dispositivo.', 'error');
            }
        }

        // Carga anticipada de modelos
        cargarFaceApiModels();
    });

    function cerrarModalRegistrarRostro() {
        if (regFacialStream) {
            regFacialStream.getTracks().forEach(t => t.stop());
            regFacialStream = null;
        }
        const videoEl = document.getElementById('reg-facial-video');
        if (videoEl) videoEl.srcObject = null;
        document.getElementById('modal-registrar-rostro')?.classList.add('hide');
    }

    document.getElementById('btn-cerrar-registrar-rostro')?.addEventListener('click', cerrarModalRegistrarRostro);

    // Registro facial en 3D con redes neuronales
    document.getElementById('btn-guardar-rostro')?.addEventListener('click', async () => {
        const videoEl = document.getElementById('reg-facial-video');
        const btnSave = document.getElementById('btn-guardar-rostro');
        const nombre = document.getElementById('reg-rostro-nombre')?.value.trim();

        if (!nombre) {
            notify('Ingresa un nombre o etiqueta para este rostro.', 'error');
            return;
        }

        btnSave.disabled = true;
        btnSave.textContent = '⏳ Cargando IA Face ID...';

        const modelsReady = await cargarFaceApiModels();
        if (!modelsReady) {
            notify('Error cargando modelos de IA.', 'error');
            btnSave.disabled = false;
            btnSave.textContent = '📸 Registrar Face ID (3 Ángulos)';
            return;
        }
        ...
        const poses = [
            { id: 'pose-step-1', msg: '📸 Paso 1/3: Mira directamente al frente...' },
            { id: 'pose-step-2', msg: '📸 Paso 2/3: Gira levemente la cabeza a la izquierda...' },
            { id: 'pose-step-3', msg: '📸 Paso 3/3: Gira levemente la cabeza a la derecha...' }
        ];

        for (let i = 0; i < poses.length; i++) {
            const p = poses[i];
            btnSave.textContent = p.msg;
            const stepEl = document.getElementById(p.id);
            if (stepEl) stepEl.className = 'pose-step activo';

            let captured = false;
            for (let retry = 0; retry < 35; retry++) {
                await new Promise(r => setTimeout(r, 200));
                const resDeep = await obtenerDescriptorFacialDeep(videoEl);
                if (resDeep) {
                    descriptors.push(resDeep.descriptor);
                    captured = true;
                    if (stepEl) stepEl.className = 'pose-step completado';
                    vibrar(25);
                    break;
                }
            }
            ...
            if (!captured) {
                notify(`Paso ${i + 1} falló. Mantén tu rostro bien iluminado y enfocado.`, 'warn');
                btnSave.disabled = false;
                btnSave.textContent = '📸 Registrar Face ID (3 Ángulos)';
                return;
            }
        ...
        if (videoEl && videoEl.videoWidth) {
            ...
            tctx.drawImage(videoEl, (videoEl.videoWidth - minDim) / 2, (videoEl.videoHeight - minDim) / 2, minDim, minDim, 0, 0, 96, 96);
        }
        const fotoRef = thumbCanvas.toDataURL('image/jpeg', 0.85);
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                notify(`¡Face ID de "${nombre}" configurado con éxito en 3D!`, 'ok');
                cerrarModalRegistrarRostro();
                cargarRostrosAdmin();
            } else {
                notify(data.message || 'No se pudo guardar el rostro.', 'error');
            }
        } catch (err) {
            notify('Error de conexión al guardar rostro.', 'error');
        }
    });
