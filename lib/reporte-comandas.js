...
function construirPdfComandas(datos) {
  ...
  const validasConCobro = validas.filter(c => !c.es_cortesia);
  const cortesias = validas.filter(c => c.es_cortesia);
  const anuladas = comandas.filter(c => c.estado_pago === 'ANULADO');

  const totalRecaudado = validasConCobro.reduce((s, c) => s + (Number(c.total) || 0), 0);
  const totalCortesia = cortesias.reduce((s, c) => s + (Number(c.total) || 0), 0);
  ...
  doc.indicadores([
    ...
    { etiqueta: 'COBRADAS', valor: String(validasConCobro.length), color: COLOR.ok },
    { etiqueta: 'CORTESÍAS', valor: String(cortesias.length), color: cortesias.length > 0 ? COLOR.tinta : COLOR.suave },
  ...
  if (cortesias.length > 0) {
    doc.parrafo(
      `* Nota: Las ${cortesias.length} comandas de cortesía (valorizadas en ${num2(totalCortesia)} Bs.) NO suman a lo recaudado en caja (0.00 Bs.).`,
      { color: COLOR.tinta, tamano: 8 }
    );
    doc.espacio(4);
  }
  ...
  if (comandas.length === 0) {
    ...
    const filas = comandas.map(c => {
      ...
      const esCortesia = !esAnulada && Boolean(c.es_cortesia);
      ...
      const meseroTxt = (c.nombre_mesero || 'Mesero') + (esCortesia ? ' [Cort.]' : '');
      ...
      if (c.detalles && c.detalles.length > 0) {
        c.detalles.forEach(d => {
          ...
          if (d.id_detalle_padre) {
            lineasProds.push(`   └> ${cant}x ${nom} (incluido)`);
          } else {
      ...
      if (esCortesia) {
        colPago = 'CORTESIA\n(0.00 Bs.)';
      } else if (c.pagos && c.pagos.length > 0) {
        const lineasPagos = [];
        c.pagos.forEach(p => {
      ...
      const colEstado = esAnulada
        ...
        : (esCortesia
            ? { texto: 'CORTESÍA', color: COLOR.suave, fuente: 'negrita' }
      ...
      const colTotal = esAnulada
        ...
        : (esCortesia
            ? { texto: '0.00 (Cort.)', color: COLOR.suave, fuente: 'monoNegrita' }
